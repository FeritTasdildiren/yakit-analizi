...
db_color = "gray"
try:
    # Basit bir query denemesi (cache kullanmadan)
    check = get_latest_mbe("benzin")
    if check:
        db_status = "BAĞLI ✅"
        db_color = "green"
    else:
        # Veri yok ama hata vermedi
...
if api_status:
    ...
    col2.markdown(f"### API Durumu\n<h2 style='color:red'>ERİŞİLEMEZ ❌</h2>", unsafe_allow_html=True)
    col2.caption("Backend servisi çalışmıyor olabilir (http://localhost:8000).")
...
# Hassas bilgileri gizleyerek goster
safe_settings = {
...
# --- Loglar (Simule) ---
# Gercek loglari okumak icin dosyadan okuma yapilabilir ama simdilik placeholder.
